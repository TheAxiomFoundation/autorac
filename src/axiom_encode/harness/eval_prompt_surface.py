...
- In `.test.yaml` for `entity: Payment` outputs, provide per-payment rows under `tables:` and assert the entity output as a row-ordered YAML list rather than flattening payment facts into scalar `input:` values.
- In `.test.yaml` for `entity: Asset` outputs, provide per-item rows under `tables:` and assert the entity output as a row-ordered YAML list rather than flattening asset facts into scalar `input:` values.
...
- In `.test.yaml` for those complete-band money rules, include a non-exact-multiple excess case like `£750` above threshold so the tests expose any accidental fractional division.
...
- When RuleSpec lacks a native date-valued output for such a temporal leaf, model the statutory day as a fact-shaped boolean helper on `period: Day`, and derive the principal output from that helper plus any explicit trigger conditions stated in the source text.
...
- In `.test.yaml` for those claim-date clauses, include one no-supersession case for the operative claim date and one supersession case for the supersession date; do not rely on paired tests that separately assert `made` and `treated as made` as independent true branches of the same reference-day selector.
...
- In those comparative month-apart phrases, the `one month` comparator is not a standalone numeric scalar that needs its own RuleSpec rule for grounding. Satisfy grounding with the semantic boolean itself; do not invent `1`-valued threshold/count helpers just to mirror the word `one`.
...
- When a fixed supplement, allowance, or addition is payable only while an eligibility condition holds, do not leave that money output unconditional.
- For those conditional amount leaves, do not collapse source-stated component facts into an opaque local input like `*_eligible_for_*`; prefer direct facts like `client_is_pregnant_parent` when the source names that fact.
...
- In those branch-specific count or basis-selector outputs, do not write an inline conditional without `else` just to avoid a sibling fallback. Either keep the output unconditional and rely on the `_applies` boolean, or use a fully valid RuleSpec conditional with an explicit `else`.
...
- For those editorially omitted dotted-text slices, keep the embedded source/docstring showing the omission, emit no local rule blocks, and leave `.test.yaml` empty rather than inventing a guessed legal effect.
...
- In `.test.yaml` for those delegated-computation slices, include separate cases for the distinct cited alternatives, such as one regulation-13 paragraph-(4) path and one paragraph-(10) path, rather than only a generic applies/does-not-apply toggle.
- In `.test.yaml` for those route-selection slices, avoid a false case that makes a self-employed-earner branch fail merely because neither local route flag was selected; that is an impossible or unresolved routing state, not a source-stated inapplicable case.
...
- In `.test.yaml` for those residual limbs, include a case where the parent conditions hold but the residual `other case` condition is false, so the branch remains inoperative.
...
- In `.test.yaml`, use helper/input names that expose the actual legal facts from the source text. Prefer names like `child_benefit_is_only_person`, `child_benefit_is_elder_or_eldest_person`, `claimant_has_partner`, `is_single_claimant`, `is_joint_claimant`, `resident_in_greater_london`, or `responsible_for_child_or_qualifying_young_person`.
- In `.test.yaml`, avoid opaque placeholders like `*_condition`, `*_eligibility_flag`, or `family_has_partner` when a more direct legal-fact name is available from the source text.
- In `.test.yaml` for those claimant-or-partner disjunctions, include a partnered case where only the claimant satisfies the source-stated condition; that case should still evaluate true.
- In `.test.yaml`, do not feed the same legal period through contradictory units or categories, such as asserting that one payment period is both `3 months` and `3 weeks` via separate free inputs.
- In `.test.yaml` for one determination limb of a larger multi-limb calculation, when the limb does not apply, assert the limb-specific applicability boolean and omit assertions that would force the principal money or rate output to equal a sibling limb's unknown result.
- For whole-provision slices phrased like `Where X, Y must ...`, encode the triggered requirement itself. Include a `.test.yaml` case where `X` is false; unless `./source.txt` expressly says otherwise, the requirement should evaluate as satisfied or inapplicable rather than false in that case.
...
- For a one-row fixed-amount slice, Do not emit malformed date blocks like `from 0:`.
- For a one-row fixed-amount slice, use exactly one grounded `effective_from: 'YYYY-MM-DD'` version unless `./source.txt` itself states multiple grounded dates or amounts.
- For a one-row fixed-amount slice, the principal amount rule should usually be a grounded constant formula under a single `effective_from`; do not wrap it in a conditional formula unless `./source.txt` itself states a second grounded amount or branch inside the same row.
...
- In `.test.yaml` for a one-row fixed-amount slice, use boolean or fact-shaped helper inputs that mirror the row text.
...
- For a one-row fixed-amount slice with a single canonical subject, keep `.test.yaml` outputs scalar instead of nested wrappers like `{person: 1, value: ...}`.
- For a one-row fixed-amount slice, every `.test.yaml` case should keep the row-defining conditions satisfied; do not negate them in alternate tests unless `./source.txt` states another grounded amount for that alternate branch.
...
- For a one-row fixed-amount slice with non-annual periods, the allowed `.test.yaml` shapes are base case and effective-date boundary.
...
- Do not use thousands separators in RuleSpec numeric literals or `.test.yaml` outputs; write `2500`, not `2,500`.
"""
...
def render_date_silent_scaffold_guidance() -> str:
    ...
    return """
- If `./source.txt` is silent on effective dates and no copied precedent file provides a scaffold date, you may use `effective_from: '0001-01-01'` as a neutral scaffold date solely to satisfy RuleSpec syntax.
