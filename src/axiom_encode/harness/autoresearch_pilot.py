...
from .eval_prompt_surface import AUTORESEARCH_EDITABLE_FILES
...
def axiom_encode_repo_root() -> Path:
...
def program_path(repo_root: Path | None = None) -> Path:
    ...
    root = repo_root or axiom_encode_repo_root()
...
def shared_legislation_cache_root() -> Path:
    ...
    return (Path.home() / "tmp" / "axiom_encode-shared-legislation-cache").resolve()
...
def pilot_editable_paths(repo_root: Path | None = None) -> list[Path]:
    ...
    root = repo_root or axiom_encode_repo_root()
    return [(root / rel).resolve() for rel in AUTORESEARCH_EDITABLE_FILES]
...
def build_mutation_prompt(
    *,
    ...
    return dedent(
        f"""
        You are running one autoresearch iteration for Axiom Encode.
    ...
    *,
    ...
    if search_base.exists():
        run_dirs = sorted(
            (
                path
                for path in search_base.glob("axiom_encode-*")
