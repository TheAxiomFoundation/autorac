...
- For a delegated boolean slice targeting `snap_state_using_standard_utility_allowance`, if the cited source publishes multiple distinct SNAP utility allowance categories for the same jurisdiction, that means the jurisdiction does not always use the standard utility allowance. Encode `snap_state_using_standard_utility_allowance` as `false` for that jurisdiction, and keep any category-specific allowance amounts only as separate helpers if useful.
...
- For example, a Texas bulletin that separately publishes `SUA`, `BUA`, and `Telephone Standard` amounts should set `snap_state_using_standard_utility_allowance` to `false`, not `true`.
