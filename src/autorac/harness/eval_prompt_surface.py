...
- In those pure cross-reference computational slices, import the actual cited canonical rule if it is present in the workspace. If the cited canonical rule is absent and the local slice does not itself state a grounded computation, prefer a compileable top-level `status: deferred` fallback over a fake local boolean encoding.
- In `.rac.test` for those pure cross-reference computational slices, if the file falls back to top-level `status: deferred`, leave the `.rac.test` file empty rather than inventing route-selection assertions.
